...
window.addEventListener('scroll', () => {
    ...
    if (currentScroll > 50) {
        ...
        navbar.style.boxShadow = '0 2px 20px rgba(0, 210, 255, 0.2)';
    } else {
        navbar.classList.remove('scrolled');
        navbar.style.background = 'rgba(26, 26, 26, 0.95)';
        navbar.style.boxShadow = '0 2px 20px rgba(0, 210, 255, 0.1)';
    }
...
async function init3DEnhancements() {
    try {
        ...
        if (hero) {
            ...
            if (window.VANTA && window.VANTA.FOG) {
                ...
                window.vantaEffect = VANTA.FOG({
                    ...
                    minHeight: 200.00,
                    minWidth: 200.00,
                    highlightColor: 0xff0080,
                    midtoneColor: 0x00d2ff,
        ...
        console.error('Failed to load tilt or vanta effects:', error);
    }
}
...
function createFlyingEnvelope(startX, startY) {
    const envelope = document.createElement('div');
    envelope.className = 'flying-envelope';
    envelope.innerHTML = '✉️'; // You can use an icon here
    ...
    requestAnimationFrame(() => {
        ...
        const cartIcon = document.querySelector('.cart-floating-btn');
        if (!cartIcon) {
            envelope.remove();
            return;
        }
        ...
        const destX = cartRect.left + cartRect.width / 2 - 12; // 12 is half envelope width roughly
        const destY = cartRect.top + cartRect.height / 2 - 12;

        envelope.style.transform = `translate(${destX - startX}px, ${destY - startY}px) scale(0.6)`;
        envelope.style.opacity = '0.7';

        // Add fun animation to cart icon when envelope arrives
        setTimeout(() => {
            cartIcon.style.animation = 'none';
            // Trigger reflow
            void cartIcon.offsetWidth;
            cartIcon.style.animation = 'badgeBounce 0.4s ease-out';
            envelope.remove();
        }, 800); // matches CSS transition time
    });
}

function addToCart(name, price, eventX, eventY) {
    console.log(`Adding to cart: ${name} at $${price}`);

    // trigger animation
    if (eventX && eventY) {
        createFlyingEnvelope(eventX, eventY);
    }
    ...
    // Open sidebar automatically after a small delay
    setTimeout(() => {
        if (cartSidebar && !cartSidebar.classList.contains("active")) {
            cartSidebar.classList.add("active");
        }
    }, 850); // Just after animation finishes
}
...
function setupCartListeners() {
    ...
    document.addEventListener("click", (e) => {
        ...
        if (btn) {
            ...
            if (name && price) {
                addToCart(name, price, startX, startY);
            } else {
